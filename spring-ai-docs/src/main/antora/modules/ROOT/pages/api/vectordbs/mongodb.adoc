...
== Metadata Filtering
Metadata filtering allows for more refined queries by filtering results based on specified metadata fields. This feature uses MongoDB's query language to perform filtering operations in conjunction with vector searches.
...
These operations enable complex filtering logic to be applied to metadata fields associated with documents in the vector store.
